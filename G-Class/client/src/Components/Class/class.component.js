  ...
  return (
    <div className="container-fluid">
      ...
      <div className="container">
        <div className="margin-top-bottom box">
          <h1 className="box-title  class-box">
            {classInfo.title}
            <a className="meet-a" href={`${classInfo.conference}`}>
              <img src={De} alt="meet" className="meet class-meet"></img>
            </a>
          </h1>

          <div className="class-desc">
            <p>{classInfo.description}</p>Meet Link :
            <a href={`${classInfo.conference}`}>{classInfo.conference}</a>
        ...
        {Object.size(authorInfo) > 0
          ? classworks.map(classwork => {
              if (authorInfo[classwork.author]) {
                return (
                  <div
                    className="margin-top-bottom box  classwork padding-class"
                    key={classwork._id}
                    onClick={() => {
                      if (classwork.types === 'material')
                        window.location = `/class/${classInfo._id}/m/${classwork._id}`;
