...
const People = params => {
  ...
  useEffect(() => {
    if (classInfo) {
      InfoById(classInfo.owner).then(result => {
  ...
  return (
    <div className="container-fluid">
      ...
      <div className="container">
        <h1 className="box-title stu">Teachers:</h1>
        {teachers.map(teacher => {
          if (teacher) {
            if (teacher.profile_picture) {
              return (
                <div key={teacher._id} className="boxxx ">
                  <img
                    src={`${URL}/${teacher.profile_picture.filename}`}
                    className="ppi pp"
        ...
        <h1 className="box-title  stu">Students:</h1>
        {students.map(student => {
          if (student) {
            if (student.profile_picture) {
              return (
                <div key={student._id} className="boxxx">
                  <img
                    src={`${URL}/${student.profile_picture.filename}`}
                    className="ppi pp"
