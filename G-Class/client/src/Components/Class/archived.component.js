...
const Archived = () => {
  ...
  const Unarchive = (classId, owner) => {
    if (userInfo._id === owner) {
      Axios.post(`${URL}/class/unarchive`, {
        owner,
        _class: classId,
        token: userInfo.token,
      }).then(() => (window.location = "/"));
    } else {
      Axios.post(`${URL}/class/user/unarchive`, {
        token: userInfo.token,
      ...
      <div className="container">
        {classes.map((_class) => {
          if (_class.archived || userInfo.archived_class.includes(_class._id)) {
            return (
              <div className="box class box-shadow" key={_class._id}>
                <div onClick={() => (window.location = `/class/${_class._id}`)}>
                  <h1 className="box-title class-box">{_class.title}</h1>
                ...
                {!_class.archived || _class.owner === userInfo._id ? (
                  <p
                    className="box-option link class-desc"
                    onClick={() => Unarchive(_class._id, _class.owner)}
                  >
                    Unarchive
                  </p>
                ) : null}
