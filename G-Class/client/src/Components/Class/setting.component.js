...
const ClassSetting = params => {
  ...
  const toTeacher = studentId => {
    Axios.post(`${URL}/class/teacher/add`, {
      owner: classInfo.owner,
      token: userInfo.token,
      _class: classInfo._id,
      teacher: studentId,
    }).then(() => {
      InfoById(studentId).then(result => {
        setTeachers(teachers => [...teachers, result]);
  ...
  const toStudent = teacherId => {
    Axios.post(`${URL}/class/students/add`, {
      owner: classInfo.owner,
      student: teacherId,
      token: userInfo.token,
      _class: classInfo.code,
    }).then(() => {
      InfoById(teacherId).then(result => {
        setStudents(students => [...students, result]);
  ...
  const removeStudent = studentId => {
    Axios.post(`${URL}/class/students/remove`, {
      owner: classInfo.owner,
      student: studentId,
      token: userInfo.token,
      _class: classInfo._id,
  ...
  const removeTeacher = teacherId => {
    Axios.post(`${URL}/class/teacher/delete`, {
      owner: userInfo._id,
      teacher: teacherId,
      _class: classInfo._id,
      token: userInfo.token,
  ...
  return (
    <div className="container-fluid">
      ...
      <div className="container">
        <form
          className="margin-top-bottom box set-box"
          onSubmit={updateClassInfo}
        >
          ...
          <div className="form-group">
            <input
              type="submit"
              className="form-control btn add-classwork-btn"
            />
          </div>
        </form>
        <div className="margin-top-bottom box set-box">
                  ...
                  <li key={teacher._id}>
                    ...
                    <ul>
                      <li>
                        <p
                          className="link"
                          onClick={() => toStudent(teacher._id)}
                        >
                          Demote become a student
                        </p>
                      </li>
                      <li>
                        <p
        ...
        <div className="margin-top-bottom box set-box">
          ...
          <ul>
            {students.length > 0 ? (
                  ...
                  <li key={student._id}>
                    ...
                    <ul>
                      <li>
                        <p
                          className="link"
                          onClick={() => toTeacher(student._id)}
                        >
                          Promote become a teacher
                        </p>
                      </li>
                      <li>
                        <p
              ...
              <h3>
                There is no students yet. Send class code({classInfo.code}) to
                your student.
              </h3>
            )}
          </ul>
        </div>
        <form className="margin-top-bottom box set-box" onSubmit={ArchiveClass}>
