...
router.post('/create', jsonParser, (req, res) => {
  const { title, conference, description, owner, token } = req.body;
        ...
        if (err) res.status(500).json('Something went wrong.');
          ...
          Class.findOne({ _id: _class }, (err, __class) => {
            if (err) res.status(500).json('Something went wrong');
              ...
              if (String(__class.owner) === teacher)
                res
                  .status(400)
                  .json('The user already has a role in this class.');
              else {
                if (__class.students.includes(teacher)) {
                  for (let i = 0; i < __class.students.length; i++) {
                    if (__class.students[i] === teacher) {
        ...
        if (err) res.status(500).json('Something went wrong');
          ...
          if (__class.teacher.includes(teacher)) {
            for (let i = 0; i < __class.teacher.length; i++) {
              if (__class.teacher[i] === teacher) {
                __class.teacher.splice(i, 1);
                i--;
              }
            }
          }
    ...
    if (err) res.status(500).json('Something went wrong.');
      ...
      Class.findOne({ code: _class }, (err, __class) => {
        if (err) res.status(500).json('Something went wrong');
          ...
          if (
            String(__class.owner) === student ||
            __class.teacher.includes(student)
          )
            res.status(400).json('The user already has a role in this class.');
        ...
        if (err) res.status(500).json('Something went wrong.');
          ...
          Class.findOne({ code: _class }, (err, __class) => {
            if (err) res.status(500).json('Something went wrong');
              ...
              if (String(__class.owner) === student)
                res
                  .status(400)
                  .json('The user already has a role in this class.');
              else {
                ...
                __class
                  .save()
                  .then(() =>
                    res.json({ message: 'Success', classId: __class._id })
                  )
                  .catch(() => res.status(400).json('Something went wrong.'));
              }
            }
...
router.post('/user/archive', jsonParser, (req, res) => {
  ...
  Class.findOne(
    { students: { $in: [student] }, _id: _class },
    (err, __class) => {
      if (err) res.status(500).json('Something went wrong.');
      else if (!__class) {
        Class.findOne(
          { teacher: { $in: [student] }, _id: _class },
          (err, __class) => {
            if (err) res.status(500).json('Something went wrong.');
            else if (!__class) res.status(400).json('Class not found');
            else {
      ...
      if (err) res.status(500).json('Something went wrong.');
                ...
                if (err) res.status(500).json('Something went wrong.');
                  ...
                  if (user.archived_class.includes(__class._id)) {
                    for (let i = 0; i < user.archived_class.length; i++) {
                      if (
                        String(user.archived_class[i]) === String(__class._id)
                      ) {
                        user.archived_class.splice(i, 1);
                        i--;
                      }
                    }
                  }
        ...
        User.findOne({ _id: student, token }, (err, user) => {
          if (err) res.status(500).json('Something went wrong.');
            ...
            if (user.archived_class.includes(__class._id)) {
              for (let i = 0; i < user.archived_class.length; i++) {
                if (String(user.archived_class[i]) === String(__class._id)) {
                  user.archived_class.splice(i, 1);
                  i--;
                }
              }
