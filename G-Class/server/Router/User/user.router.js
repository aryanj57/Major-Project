      ...
      User.findOne({ username }, (err, user) => {
        if (err) res.status(500).json('Error has occured. Please refresh page');
          ...
          newUser
            ...
            .catch(err =>
              res.status(500).json('Error has occured. Please refresh page')
            );
        }
      });
    }
...
router.post('/googlelogin', (req, res) => {
  const { email, name, isGoogle } = req.body;
  //find the user
  if (isGoogle == true) {
    User.findOne({ email }, (err, user) => {
      // if (err) res.status(500).json('Error has occured.');
  ...
  upload(req, res, () => {
    ...
    User.findOne({ token: req.body.token }, (err, user) => {
      if (user.profile_picture) {
        fs.unlink(
          user.profile_picture.destination + user.profile_picture.filename,
          err => {
